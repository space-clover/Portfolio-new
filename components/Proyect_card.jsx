...
const ProyectCard = ({ titulo, descripcion, etiquetas, IMG, src, alt }) => {
    ...
    return (
        ...
        hover:shadow-md hover:shadow-slate-400  transition duration-300 rounded-xl my-4'> 
            <div className="lg:w-1/4 px-2 flex items-center justify-center"> {IMG}  </div>
            <div className='lg:w-3/4'>
                <h3 className='text-lg py-1/100 flex items-center text-gray-800 font-bold'>{titulo} 
                ...
                <ul className='flex  w-full mt-3 flex-wrap '>
                    {etiquetas.map((etiqueta, index) => (
                        <li className=' px-3 py-0.5 text-sm rounded-lg mr-2 mb-2 bg-cyan-950 text-slate-100 bg-opacity-75' key={index}>{etiqueta}</li>
                    ))}
                </ul>
            </div>
        </motion.div>
